...
namespace Ylb\Validate\Validate;

use Hyperf\DbConnection\Db;
use Ylb\Validate\Exception\ValidateException;
use File;
...
//                $result = $this->checkItem($key, $value, $rule->getRule(), $data, $rule->getTitle() ?: $title, $rule->getMsg());
     ...
     */
    public function makeCheck($data, $scene = '', $filter = false, $throw = false, $batch = false)
    {
        $rules = $this->rule;
        // 分析验证规则
        if (!empty($scene) && isset($this->scene[$scene])) {
            // 如果设置了验证适用场景
            $sceneArr = $this->scene[$scene];
        } else {
            $sceneArr = $this->rule;
        }
        $error = [];
        $filterData = [];
        if (is_array($sceneArr)) {
            // 处理场景验证字段
            $change = [];
            $array = [];
            foreach ($sceneArr as $k => $val) {
                if (is_numeric($k)) {
                    $array[] = $val;
                    if ($filter && isset($data[$val])) $filterData[$val] = $data[$val];
                } else {
                    $array[] = $k;
                    $change[$k] = $val;
                }
        ...
        // 如果全部为空的话，则证明不验证参数，如果开启过滤就需要过滤一下数据
        foreach ($rules as $key => $item) {
            ...
            if ($rule instanceof \Closure) {
                ...
                $result = $this->makeCheckItem($key, $value, $rule, $data, $filterData, $filter, $title, $msg);
            }
            if (true !== $result) {
                // 没有返回true 则表示验证失败
                if (!empty($batch)) {
            ...
            if ($filter) $filterData[$key] = $value;
        }
        if ($filter && $throw) {
            foreach ($data as $datum => $val1) {
                if ((!isset($filterData[$datum])) && !is_array($val1)) {
                    throw new ValidateException($datum . ' 参数无效！');
                }
                if (is_array($val1)) {
                    foreach ($val1 as $key2 => $val2) {
                        $vk = $datum . '.' . $key2;
                        if ((!isset($filterData[$vk]))) throw new ValidateException($datum . '.' . $key2 . ' 参数无效！');
                    }
                }
            }
        }
        unset($result, $error, $rule, $rules);
     ...
     * @return mixed
     */
    private function makeCheckItem($field, $value, $rules, $data, $filterData, $filter, $title = '', $msg = [])
    {
    ...
    public function is($value, $rule, $data = [])
    {
        ...
        switch (lcfirst($rule)) {
            ...
            case 'email':
                ...
                break;
            case 'token':
                $result = false;//$this->token($value, '__token__', $data);
                break;
            default:
     ...
     * @return bool
     */
    protected function unique($value, $rule, $data, $field, $title)
    {
        if (is_string($rule)) {
            $rule = explode(',', $rule);
        }
        ...
        if (strpos($key, '^')) {
            ...
            foreach ($fields as $key) {
                if (isset($data[$key])) {
                    $db->where($key, $data[$key]);
                }
            }
        } elseif (strpos($key, '=')) {
            parse_str($key, $map);
        } elseif (isset($data[$field])) {
            $db->where($key, $data[$field]);
        } else {
            $map = [];
        }
        $pk = isset($rule[3]) ? $rule[3] : 'id';
        if (is_string($pk)) {
            if (isset($rule[2]) && $rule[2] != 0) {
                $map[] = [$pk, '!=', $rule[2]];
